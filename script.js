...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (isHome) {
    ...
    if (locoEl && typeof LocomotiveScroll !== "undefined") {
      ...
      locoScroll.on("scroll", ({ scroll, limit }) => {
        ...
        if (thumb && limit.y > 0) {
          const height = (window.innerHeight / limit.y) * 100;
          thumb.style.height = height + "vh";
          thumb.style.top = (scroll.y / limit.y) * (100 - height) + "vh";
        }
  ...
  /* ================================================================
     7. HERO WEBGL FLUID DISTORTION
  ================================================================ */
  const glWrap = document.getElementById("hero-gl");
  if (glWrap && typeof THREE !== "undefined") {
    ...
    const material = new THREE.ShaderMaterial({
      uniforms: {
        ...
        uMouse: { value: mouse },
        uTime: { value: 0 },
        uRatio: { value: 1 } /* canvas w/h — set properly in resize() */,
        uImgRatio: { value: 1.5 } /* image  w/h — set on texture load       */,
      },
      vertexShader: `
        varying vec2 vUv;
        void main() {
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          vec2 scale = vec2(1.0);
          if (uRatio > uImgRatio) {
            scale.y = uImgRatio / uRatio;
          } else {
            scale.x = uRatio / uImgRatio;
          }
          uv = (uv - 0.5) / scale + 0.5;

          /* distortion — only active when uTime > 0 */
          vec2 diff      = uv - uMouse;
          float dist     = length(diff);
          float strength = smoothstep(0.35, 0.0, dist);
          uv += diff * strength * 0.07 * sin(uTime * 2.0);
    ...
    /* Load texture AFTER material exists so callback can set uImgRatio */
    const texture = new THREE.TextureLoader().load(
      "img/charlotte-noelle-98WPMlTl5xo-unsplash.jpg",
      (tex) => {
        material.uniforms.uTexture.value = tex;
        material.uniforms.uImgRatio.value = tex.image.width / tex.image.height;
      },
    );
    ...
    /* ── resize: read actual DOM size, guard against 0×0 ── */
    function glResize() {
      const W = glWrap.offsetWidth;
      const H = glWrap.offsetHeight;
      if (W === 0 || H === 0) return;
      renderer.setSize(W, H);
      renderer.domElement.style.width = "100%";
      renderer.domElement.style.height = "100%";
      material.uniforms.uRatio.value = W / H;
    }

    /* Run after full page paint so layout is complete */
    window.addEventListener("load", glResize, { once: true });
    window.addEventListener("resize", glResize, { passive: true });
    /* Also try immediately in case load already fired */
    glResize();

    /* Mouse tracking — only inside the wrap */
    glWrap.addEventListener("mousemove", (e) => {
    ...
    glWrap.addEventListener("mouseenter", () => {
      isHovering = true;
    });
    glWrap.addEventListener("mouseleave", () => {
      isHovering = false;
      target.set(0.5, 0.5);
    });
    ...
    (function glLoop() {
      ...
      mouse.lerp(
        isHovering ? target : new THREE.Vector2(0.5, 0.5),
        isHovering ? 0.06 : 0.04,
      );
      material.uniforms.uTime.value = isHovering ? clock.getElapsedTime() : 0;
